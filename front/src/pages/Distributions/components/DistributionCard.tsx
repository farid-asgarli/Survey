...
export function DistributionCard({ distribution, onView, onSend, onCancel, onDelete, isSending }: DistributionCardProps) {
  ...
  const clickRate = distribution.openedCount > 0 ? Math.round((distribution.clickedCount / distribution.openedCount) * 100) : 0;
      ...
      <CardContent className="p-5">
        ...
        <div className="flex items-start gap-3 mb-4">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="font-semibold text-on-surface line-clamp-1 leading-tight">
              {distribution.subject || t('distributions.untitledDistribution')}
            </h3>
        ...
        {/* Metrics for Sent distributions */}
        {isSent && (
          <div className="space-y-3 mb-4">
            ...
            <div>
              <div className="flex justify-between items-center mb-1.5">
                <span className="text-sm text-on-surface-variant">{t('distributions.clickRate')}</span>
                <span className="text-sm font-semibold text-primary">{clickRate}%</span>
              </div>
              <div className="h-2 w-full overflow-hidden rounded-full bg-primary/15">
                <div className="h-full rounded-full bg-primary transition-all duration-500" style={{ width: `${clickRate}%` }} />
              </div>
            </div>
          </div>
        )}
