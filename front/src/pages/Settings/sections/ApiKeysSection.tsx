...
import { useDialogState, useCopyToClipboard, useDateTimeFormatter } from '@/hooks';
    ...
    <div className='space-y-6'>
        ...
        <CardContent>
          {apiKeys.length === 0 ? (
            ...
            <div className='space-y-3'>
              {apiKeys.map((key) => (
                <div key={key.id} className='flex items-center justify-between p-4 rounded-2xl bg-surface-container-lowest border border-outline-variant/30'>
      ...
      <Dialog open={createDialog.isOpen} onOpenChange={createDialog.setOpen}>
        <DialogContent className='sm:max-w-md' showClose={false}>
          <DialogHeader hero icon={<Key className='h-7 w-7' />} title={t('settings.security.createApiKey')} description={t('apiKeys.createDesc')} showClose />
