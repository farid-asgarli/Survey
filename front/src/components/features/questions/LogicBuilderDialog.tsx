...
import type { QuestionLogic, CreateLogicRequest } from '@/types';
...
export function LogicBuilderDialog({ open, onOpenChange, questionId, surveyId }: LogicBuilderDialogProps) {
  ...
  const handleSaveRule = async () => {
    ...
    const data: CreateLogicRequest = {
      sourceQuestionId: formData.sourceQuestionId,
      operator: formData.operator,
      conditionValue: needsValueInput ? formData.conditionValue : '',
    ...
    try {
      if (editingRule) {
        await updateLogic.mutateAsync({ logicId: editingRule.id, data });
      } else {
        await createLogic.mutateAsync(data);
      }
      handleCancelEdit();
    } catch (error) {
