...
function RunHistoryItem({ run }: { run: RecurringRun }) {
  ...
  return (
    <div className='flex items-start gap-3 p-4 bg-surface-container-low rounded-2xl'>
      {/* Status icon */}
      <div
        className={cn('flex h-10 w-10 items-center justify-center rounded-xl shrink-0', run.status === RunStatus.Running && 'animate-pulse', status.className)}
      >
...
export function RunHistoryDrawer({ open, onOpenChange, recurringSurvey }: RunHistoryDrawerProps) {
  ...
  const stats = useMemo(() => {
    ...
    const cancelled = runs.filter((r) => r.status === RunStatus.Cancelled).length;
    const totalResponses = runs.reduce((sum, r) => sum + r.responsesCount, 0);
    return { completed, failed, cancelled, totalResponses };
  }, [runs]);
        ...
        {runs.length > 0 && (
          <div className='px-5 pt-4 pb-2'>
            <div className='grid grid-cols-4 gap-2'>
              ...
              <div className='text-center p-2 bg-surface-container/50 rounded-xl'>
                <p className='text-lg font-bold text-on-surface-variant'>{stats.cancelled}</p>
        ...
        <DrawerBody>
          {isLoading ? (
            ...
            <div className='space-y-3'>
              ...
              {hasNextPage && (
                <div className='pt-2'>
                  <Button variant='tonal' className='w-full' onClick={() => fetchNextPage()} disabled={isFetchingNextPage} loading={isFetchingNextPage}>
                    {t('common.loadMore')}
                  </Button>
                </div>
              )}
