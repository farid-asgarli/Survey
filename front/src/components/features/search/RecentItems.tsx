...
import { getSearchTypeIcon, getSearchTypeColors } from '@/config';
import { Clock, X, Trash2, FileText } from 'lucide-react';
...
function RecentItemRow({ item, onSelect, onRemove, compact = false }: RecentItemRowProps) {
  ...
  const colorClasses = getSearchTypeColors(item.type, true);
  ...
  return (
    ...
    >
      {/* Icon */}
      <div className={cn('flex items-center justify-center shrink-0 rounded-xl', compact ? 'h-8 w-8' : 'h-10 w-10', colorClasses)}>
