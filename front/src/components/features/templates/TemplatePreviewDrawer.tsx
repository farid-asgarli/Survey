...
function QuestionPreviewItem({
  ...
  return (
    <div className='flex items-start gap-3'>
      <span className='flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-primary/10 text-primary text-xs font-medium'>{index + 1}</span>
  ...
  return (
    <Drawer open={open} onOpenChange={onOpenChange} side='right'>
      <DrawerContent className='w-full max-w-md' showClose={false}>
        ...
        {template && !isLoading && <TemplatePreviewContent template={template} onUseTemplate={handleUseTemplate} onClose={() => onOpenChange(false)} />}
      </DrawerContent>
    </Drawer>
  );
}
