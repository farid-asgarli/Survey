...
function savedThemeToUnified(theme: SurveyTheme): UnifiedTheme {
  const primary = theme.colors?.primary || theme.primaryColor || '#0066FF';
  const secondary = theme.colors?.secondary || theme.secondaryColor || '#00A3FF';
  const background = theme.colors?.background || theme.backgroundColor || '#FAFCFF';
  const accent = theme.colors?.accent || '#E8F4FF';
  const surface = theme.colors?.surface || '#FFFFFF';
  const textPrimary = theme.colors?.text || theme.textColor || '#111827';
  const textSecondary = theme.colors?.onSurfaceVariant || '#6B7280';
  const error = theme.colors?.error || '#DC2626';
  const success = theme.colors?.success || '#16A34A';

  // Normalize font family from backend format to frontend CSS format
  const fontFamily = normalizeFontFamily(theme.typography?.fontFamily || theme.fontFamily);
  const headingFontFamily = normalizeFontFamily(theme.typography?.headingFontFamily || theme.typography?.fontFamily || theme.fontFamily);
  ...
  return {
    ...
    fontFamily,
    headingFontFamily,
    cornerRadius: '12px', // Default, could be extended if backend supports it
    spacing: 'normal',
    containerWidth: '800px',
    progressBarStyle: 'linear',
    isDark: theme.isDark, // Use backend-computed value
