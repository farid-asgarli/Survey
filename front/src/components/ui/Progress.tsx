...
function LinearProgress({ className, value = 0, indeterminate = false, size = 'default', ref, ...props }: LinearProgressProps) {
...
function CircularProgress({ className, value = 0, indeterminate = false, size = 'default', strokeWidth, ref, ...props }: CircularProgressProps) {
  const { t } = useTranslation();
  const sizeClasses = {
    sm: 'h-5 w-5',
    default: 'h-10 w-10',
...
function LoadingIndicator({ className, size = 'default', label, fullScreen = false, ref, ...props }: LoadingIndicatorProps) {
  const content = (
    <div ref={ref} className={cn('flex flex-col items-center justify-center gap-3', className)} {...props}>
      ...
      {label && (
        <p className={cn('text-on-surface-variant', size === 'sm' && 'text-xs', size === 'default' && 'text-sm', size === 'lg' && 'text-base')}>{label}</p>
      )}
    </div>
  );
