...
import { Save, Eye, Undo2, Redo2, Settings, Check, Loader2, Palette, Lock, Pencil } from 'lucide-react';
        ...
        <div className='flex items-center gap-3'>
          ...
          <div className='flex flex-col'>
            {isReadOnly ? (
              ...
              <div className='group/title relative flex items-center gap-1.5'>
                <input
                  type='text'
                  value={surveyTitle}
                  onChange={(e) => onTitleChange(e.target.value)}
                  className={cn(
                    'text-base font-semibold text-on-surface bg-transparent border-none outline-none focus:ring-0 px-0 py-0',
                    'hover:text-primary focus:text-primary transition-colors cursor-text',
                    'placeholder:text-on-surface-variant/60'
                  )}
                  placeholder={t('surveyBuilder.untitledSurvey')}
                />
                <Pencil className='w-3.5 h-3.5 text-on-surface-variant/50 opacity-0 group-hover/title:opacity-100 group-focus-within/title:opacity-100 transition-opacity shrink-0' />
              </div>
            )}
