...
// Uses @dnd-kit for unified drag-and-drop: palette-to-canvas and block reordering
import { useState, useCallback, useMemo, useRef, useEffect } from 'react';
import { useTranslation } from 'react-i18next';
import { DndContext, DragOverlay, MeasuringStrategy, defaultDropAnimationSideEffects } from '@dnd-kit/core';
...
import { BlockPalette, PaletteItemOverlay } from './BlockPalette';
...
import { usePaletteToCanvasDnd } from './usePaletteToCanvasDnd';
import { SortableBlock, BlockOverlay, DroppableCanvas, DropIndicator, CanvasDropIndicator, EdgeDropZone } from './SortableBlock';

// Drop animation configuration for DragOverlay
...
export function VisualEmailEditor({ template, onBack, onSaved }: VisualEmailEditorProps) {
  ...
  // Block operations - addBlock needs to be defined early for the DnD hook
  const addBlockWithHistory = useCallback(
    ...
    [blocks, history, historyIndex, t]
  );

  // Use unified @dnd-kit hook for palette-to-canvas and block reordering
  const dnd = usePaletteToCanvasDnd({
    blockIds: blocks.map((b) => b.id),
    onAddBlock: addBlockWithHistory,
  ...
  // Note: Native HTML5 drag removed - now using @dnd-kit via usePaletteToCanvasDnd hook
    ...
    <div className="flex flex-col h-full bg-surface-container-lowest" role="application" aria-label={t('emailEditor.title', 'Visual Email Editor')}>
      ...
      <header className="flex items-center justify-between px-4 py-3 border-b-2 border-outline-variant/30 bg-surface shrink-0" role="banner">
        <div className="flex items-center gap-3">
          {onBack && <BackButton onClick={onBack} hideTooltip />}
          <div>
      ...
      {/* Main content area - wrapped in DndContext for drag-and-drop */}
      {viewMode === 'edit' ? (
        <DndContext
          sensors={dnd.sensors}
        ...
        >
          <div className="flex-1 flex overflow-hidden">
            ...
            <aside className="w-72 border-r-2 border-outline-variant/30 bg-surface flex flex-col overflow-hidden">
              ...
              <div className="flex-1 overflow-y-auto p-3">
                {activePanel === 'blocks' && <BlockPalette onAddBlock={addBlockWithHistory} />}
            ...
            <main className="flex-1 flex flex-col overflow-hidden">
              ...
              <div className="flex-1 overflow-auto p-6" style={{ backgroundColor: globalStyles.backgroundColor }}>
                <div ref={canvasRef} className="max-w-2xl mx-auto min-h-full">
                  {/* M3 Card wrapper for email canvas with droppable zone */}
                  <DroppableCanvas
                    isActive={dnd.isDraggingFromPalette}
                    className={cn(
                      'rounded-2xl overflow-hidden border-2 transition-all duration-200',
                      dnd.isOverCanvas && dnd.isDraggingFromPalette ? 'border-primary border-dashed' : 'border-outline-variant/30'
                    )}
                    style={{
                  ...
                  >
                    {blocks.length === 0 ? (
                      <div className={cn('p-12 text-center transition-all duration-200', dnd.isOverCanvas && dnd.isDragging && 'bg-primary/10')}>
                        {dnd.isDragging && dnd.isOverCanvas ? (
                          <CanvasDropIndicator isVisible={true} isEmpty={true} />
                        ) : (
                          <>
                            <Plus
                              className={cn(
                                'h-12 w-12 mx-auto mb-4 transition-colors',
                                dnd.isOverCanvas && dnd.isDragging ? 'text-primary' : 'text-on-surface-variant/30'
                              )}
                            />
                            <p className="text-on-surface-variant">{t('emailEditor.emptyCanvas', 'Drag blocks here or click to add')}</p>
                            <Button variant="tonal" size="sm" className="mt-4" onClick={() => addBlockWithHistory('text')}>
                      ...
                      <SortableContext items={blocks.map((b) => b.id)} strategy={verticalListSortingStrategy}>
                        <div className="space-y-0">
                          ...
                          <EdgeDropZone
                            position="top"
                            isActive={dnd.isDragging}
                            showIndicator={dnd.insertionIndex === 0 && dnd.insertionPosition === 'before'}
                          />

                          {blocks.map((block, index) => {
                            // Determine if this block should show a drop indicator
                            const showBeforeIndicator =
                              dnd.isDragging && dnd.insertionIndex === index && dnd.insertionPosition === 'before' && index !== 0;
                            const showAfterIndicator = dnd.isDragging && dnd.insertionIndex === index && dnd.insertionPosition === 'after';
                            // For the last block, also check if inserting at the end
                            const isLastBlock = index === blocks.length - 1;
                            const showEndIndicator =
                              isLastBlock && dnd.isDragging && dnd.insertionIndex === blocks.length && dnd.insertionPosition === 'after';

                            return (
                              <div key={block.id} className="relative">
                                {/* Drop indicator before this block */}
                                <DropIndicator isVisible={showBeforeIndicator} position="before" />
                                ...
                                <DropIndicator isVisible={showAfterIndicator || showEndIndicator} position="after" />
                              </div>
                            );
                          })}
                          ...
                          <EdgeDropZone
                            position="bottom"
                            isActive={dnd.isDragging}
                            showIndicator={dnd.insertionIndex === blocks.length - 1 && dnd.insertionPosition === 'after'}
                          />

                          {/* Show indicator at the very end when dropping on canvas */}
                          {dnd.isDragging && dnd.insertionIndex === blocks.length && !dnd.insertionPosition && (
                            <CanvasDropIndicator isVisible={true} />
                          )}
                        </div>
                      </SortableContext>
                    )}
            ...
            {/* Right sidebar - Block Settings (inside DndContext for edit mode) */}
            <aside
              className={cn(
          ...
          {/* Drag overlay - renders dragged items outside normal flow */}
          <DragOverlay dropAnimation={dropAnimationConfig}>
            {dnd.activeDrag?.type === 'palette' && dnd.activeDrag.blockType && <PaletteItemOverlay blockType={dnd.activeDrag.blockType} />}
            {dnd.activeDrag?.type === 'block' && dnd.activeDrag.blockId && (
              <BlockOverlay block={blocks.find((b) => b.id === dnd.activeDrag!.blockId)!} />
            )}
          </DragOverlay>
        </DndContext>
      ) : (
        /* Non-edit modes (preview/code) - no DndContext needed */
        <div className="flex-1 flex overflow-hidden">
          <main className="flex-1 flex flex-col overflow-hidden">
            {/* Template metadata bar - M3 border */}
            <div className="px-5 py-3 border-b-2 border-outline-variant/30 bg-surface shrink-0">
