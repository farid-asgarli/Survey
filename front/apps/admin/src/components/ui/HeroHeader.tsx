 ...
 * ```
 */
function OverlayHeader({ icon, title, description, variant = 'primary', showClose = false, onClose, children, className, ref, ...props }: OverlayHeaderProps) {
  ...
  return (
    <div ref={ref} className={cn(bg, 'px-5 pt-5 pb-5', className)} {...props}>
      <div className='flex items-start gap-4'>
        ...
        {showClose && onClose && (
          <IconButton variant='standard' size='sm' onClick={onClose} aria-label={t('common.close')} className='-mr-2 -mt-1'>
            <X className='h-5 w-5' />
          </IconButton>
        )}
      </div>
...
function OverlayHeaderBadge({ icon, text, variant = 'success', className, ref, ...props }: OverlayHeaderBadgeProps) {
  return (
    <div ref={ref} className={cn('flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium', badgeVariantClasses[variant], className)} {...props}>
