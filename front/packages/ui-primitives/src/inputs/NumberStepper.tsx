...
interface NumberStepperProps
  extends Omit<InputHTMLAttributes<HTMLInputElement>, 'onChange' | 'size' | 'type'>, VariantProps<typeof numberStepperVariants> {
  /** Current value */
  value: number;
    ...
    <div className={cn(numberStepperVariants({ size, fullWidth, className }))}>
      ...
      <div className="flex-1 flex items-center justify-center min-w-0">
        <input
          ...
          disabled={disabled}
          className={cn(
            'w-full text-center bg-transparent text-on-surface font-medium',
            'focus:outline-none',
            '[appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none',
            size === 'sm' && 'text-xs px-1',
            size === 'default' && 'text-sm px-2',
            size === 'lg' && 'text-base px-2',
        ...
        {suffix && (
          <span
            className={cn(
              'text-on-surface-variant shrink-0 pr-2',
              size === 'sm' && 'text-xs',
