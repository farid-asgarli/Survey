...
function CalendarGrid({
  ...
  const getDateClasses = (date: Date | null) => {
    ...
    const inRange =
      ...
      !isRangeStartDay &&
      !isRangeEndDay &&
      isDateInRange(date, rangeStart, effectiveEnd);
            ...
            <div key={rowIndex} className="flex w-full">
              {rowDays.map((date, colIndex) => {
                ...
                return (
                  <div
                    key={colIndex}
                    className="relative h-12 flex-1 flex items-center justify-center"
                  >
...
function MonthYearPicker({ currentMonth, currentYear, onSelect, onBack }: MonthYearPickerProps) {
  ...
  return (
    <div className="p-4">
      {/* Year selector */}
      <div className="mb-4">
        <label className="block text-xs font-medium text-on-surface-variant mb-2 px-1">
          {t("datePicker.year")}
        </label>
...
function DatePickerDialog({
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      {/* Backdrop - M3 scrim */}
      <div
        className="fixed inset-0 bg-scrim/32 animate-in fade-in duration-200"
        onClick={onClose}
      />
        ...
        <div className="px-6 pt-5 pb-6 border-b border-outline-variant/20">
          ...
          <div className="flex items-center justify-between">
            <h2 className="text-3xl leading-10 font-normal text-on-surface">
              {selectedDate
                ? formatM3HeaderDate(formatDateISO(selectedDate), t)
                : t("datePicker.noDate")}
            </h2>
            <IconButton
              variant="standard"
              ...
              aria-label={
                viewMode === "input"
                  ? t("datePicker.switchToCalendar")
                  : t("datePicker.switchToInput")
              }
    ...
    <div className={cn("w-full", className)}>
      ...
      <button
        ...
        className={cn(
          ...
          hasError
            ? "border-error"
            : "border-outline-variant/50 hover:border-outline-variant focus:border-primary",
      ...
      >
        ...
        <span
          className={cn(
            ...
            displayValue ? "text-on-surface" : "text-on-surface-variant/40"
          )}
        >
      ...
      {(helperText || error) && (
        <p className={cn("mt-2 text-sm", hasError ? "text-error" : "text-on-surface-variant/70")}>
          {error || helperText}
        </p>
      )}
  ...
  const handleConfirm = useCallback(() => {
    if (rangeStart) {
      onChange(
        ...
        rangeEnd ? formatDateISO(rangeEnd) : formatDateISO(rangeStart)
      );
      onClose();
    }
  ...
  const displayRange = rangeStart
    ? `${formatM3HeaderDate(formatDateISO(rangeStart), t)}${
        rangeEnd && !isSameDay(rangeStart, rangeEnd)
          ? ` – ${formatM3HeaderDate(formatDateISO(rangeEnd), t)}`
          : ""
      }`
