...
export function FileUploadRenderer({
  ...
  const allowedTypes = useMemo(
    () => question.settings?.allowedFileTypes || [],
    [question.settings?.allowedFileTypes]
  );
    ...
    <div className="space-y-3">
      ...
      >
        ...
        <p className="text-on-surface-variant/70 text-sm">
          {t("questionRenderers.maxFiles", { count: maxFiles, size: maxFileSize / 1024 / 1024 })}
        </p>
      ...
      {files.length > 0 && (
        <div className="space-y-2">
          {files.map((file, index) => (
            <div
              key={index}
              className="flex items-center gap-3 p-3 rounded-xl bg-surface-container border border-outline-variant/30"
            >
