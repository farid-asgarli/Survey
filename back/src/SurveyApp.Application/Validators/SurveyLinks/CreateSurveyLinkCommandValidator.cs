...
/// <summary>
/// Validator for CreateSurveyLinkCommand.
/// </summary>
public class CreateSurveyLinkCommandValidator : AbstractValidator<CreateSurveyLinkCommand>
{
    public CreateSurveyLinkCommandValidator(
        IStringLocalizer<CreateSurveyLinkCommandValidator> localizer
    )
    {
        RuleFor(x => x.SurveyId).NotEmpty().WithMessage(localizer["Validation.Survey.IdRequired"]);

        RuleFor(x => x.Type).IsInEnum().WithMessage(localizer["Validation.SurveyLink.InvalidType"]);
