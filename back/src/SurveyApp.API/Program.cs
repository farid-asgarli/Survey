...
builder.Services.AddOutputCache(options =>
{
    ...
    options.AddPolicy(
        "Nps",
        builder =>
            builder.Expire(TimeSpan.FromMinutes(5)).Tag("survey-nps").SetVaryByRouteValue("id")
    );
    ...
    options.AddPolicy(
        "ExportPreview",
        builder =>
            builder.Expire(TimeSpan.FromMinutes(2)).Tag("export-preview").SetVaryByRouteValue("id")
    );
});
...
// Output Caching (after auth so cache varies by authenticated user when needed)
app.UseOutputCache();
